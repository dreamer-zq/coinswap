...
export class CoinSwap {
    ...
    sendSwapTx(input, output, isBuyOrder) {
        if(isBuyOrder){
            input.amount = input.amount * (1 + slippageRate)
        }else {
            output.amount = output.amount * (1 - slippageRate)
        }
        return this._sendRawTransaction("swap_order",{input,output,isBuyOrder});
    }

    sendAddLiquidityTx(maxToken,exactIrisAmt,minLiquidity){
        return this._sendRawTransaction("add_liquidity",{maxToken,exactIrisAmt,minLiquidity});
    }

    sendRemoveLiquidityTx(withdrawLiquidity,minIrisAmt,minToken){
        return this._sendRawTransaction("remove_liquidity",{withdrawLiquidity,minIrisAmt,minToken});
    }
    ...
    _sendRawTransaction(type,req){
        ...
        return this.ledger.getAddressAndPubKey().then( async account => {
            ...
            switch (type) {
                ...
                case "add_liquidity" : {
                    msg = this._createAddLiquidityMsg(account.addr,req);
                    break
                }
                case "remove_liquidity" : {
                    msg = this._createRemoveLiquidityMsg(account.addr,req);
                    break
                }
                default : {
            ...
            parent.ledger.signTx(signMsg).then( signature => {
                stdTx.SetSignature({pub_key: account.pubKey, signature: signature});
                let postData = stdTx.GetData();
                return parent.client.sendRawTransaction(postData)
            });
        });
    }
    ...
    _createRemoveLiquidityMsg(req){
        return {
            ...
            minIrisAmt: `${req.minIrisAmt}`,
            minToken: `${req.minToken}`,
        }
    }
}
